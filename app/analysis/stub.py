...
# 日付単位の締切（「本日中」「6月20日まで」等）を「その人の一日」として
# 解釈するための業務タイムゾーン. received_at は UTC で保持するため,
# 「当日いっぱい（その日の終わり）」を決めるにはローカル TZ が要る.
# 日本標準時は DST が無いので固定オフセット +09:00 で十分（tzdata 非依存）.
...
def _extract_deadline(text: str, base: datetime | None, local_tz: tzinfo) -> datetime | None:
    """本文/件名から締切日を素朴に抽出する. 見つからなければ None.

    日付単位の締切は local_tz における「その日の終わり (23:59:59)」を返す
    （= 当日いっぱい）. これにより当日中は期限切れ扱いにならない.
    明示日付（ISO・和文）を優先し, 無ければ「本日」系語があれば受信日
    （local_tz 換算）の終わりを返す.
    """

    def _end_of_day(year: int, month: int, day: int) -> datetime | None:
        try:
            return datetime(year, month, day, 23, 59, 59, tzinfo=local_tz)
        except ValueError:
            return None
    ...
    if m:
        ...
        return _end_of_day(year, month, day)

    m = _DATE_JP.search(text)
    if m:
        month, day = int(m.group(1)), int(m.group(2))
        # 和文「M月D日」は年が無いので受信年（local_tz 換算）で補完する.
        base_local = base.astimezone(local_tz) if base else datetime.now(local_tz)
        return _end_of_day(base_local.year, month, day)

    if base and any(w in text for w in _TODAY_WORDS):
        base_local = base.astimezone(local_tz)
        return _end_of_day(base_local.year, base_local.month, base_local.day)
...
class StubAnalyzer:
    ...
    def __init__(self, local_tz: tzinfo | None = None) -> None:
        # 日付単位の締切を「その日の終わり」として解釈する業務 TZ（既定 JST）.
        self._local_tz = local_tz or _JST
    ...
    def analyze(self, email: EmailMessage) -> AnalysisResult:
        ...
        deadline = _extract_deadline(f"{subject}\n{body}", email.received_at, self._local_tz)
