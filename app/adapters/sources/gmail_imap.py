...
class GmailImapSource:
    ...
    def _build_message(self, descriptor: bytes, raw: bytes, *, is_spam: bool = False) -> EmailMessage:
    ...
    def _fetch_folder(
        self, imap: imaplib.IMAP4_SSL, mailbox: str, limit: int, *, is_spam: bool
    ) -> list[EmailMessage]:
        """指定メールボックスの最新 limit 件を新しい順で返す（読み取り専用）."""
        ...
        if status != "OK":
            raise RuntimeError(f"{mailbox} を開けませんでした: {data}")
        ...
        results = [
            self._build_message(item[0] or b"", item[1], is_spam=is_spam)
    ...
    def _find_spam_mailbox(self, imap: imaplib.IMAP4_SSL) -> str:
        """迷惑メールフォルダ名を返す.

        Gmail は IMAP の特殊用途属性（RFC 6154 \\Junk）でロケールに依存せず
        迷惑メールフォルダを報告する（表示名が「[Gmail]/迷惑メール」等でも検出可能）.
        LIST 応答から見つからない場合は既定名にフォールバックする.
        """
        try:
        ...
        for mb in mailboxes or []:
            decoded = mb.decode(errors="replace") if isinstance(mb, bytes) else str(mb)
            if "\\Junk" in decoded:
    ...
    def list_recent(self, limit: int = 10) -> list[EmailMessage]:
        ...
        imap = self._connect()
        try:
            results = self._fetch_folder(imap, "INBOX", limit, is_spam=False)

            spam_mailbox = self._find_spam_mailbox(imap)
            try:
                results += self._fetch_folder(imap, spam_mailbox, limit, is_spam=True)
            except Exception:
